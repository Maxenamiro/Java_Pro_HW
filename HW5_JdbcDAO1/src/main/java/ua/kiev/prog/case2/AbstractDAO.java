...
public abstract class  AbstractDAO<T> {
    ...
    public void add(T t) {
        try {
            ...
            try (Statement st2 = conn.createStatement()) {
                try (ResultSet rs = st2.executeQuery("SELECT * FROM " + table)) {
                        while (rs.next() ) {
    ...
    public List<T> getAll(Class<T> cls,String... arrayNamesParams) {
    //public List<T> getAll(Class<T> cls, String sNamesParams) {
    ...
    // <DZ#2>
    //                  String sNamesParams1 = sNamesParams.substring(1);
    //                  String sNamesParamsRes = sNamesParams1 + ", \"";
    //                  String[] arrayNamesParam = sNamesParamsRes.split("[\", \"]");
                        for (String sNameParam:arrayNamesParams) {
                            for (int i = 1; i <= md.getColumnCount(); i++) {
                                String columnName = md.getColumnName(i);
                                if (columnName.equals(sNameParam)) {
    // </DZ#2>
